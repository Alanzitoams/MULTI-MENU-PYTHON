...
#PROGRAMA DE CRIAÇÃO DE MENU PARA OPERAÇÕES MATEMÁTICAS


#def criada somente para a entrada de dois numeros nas opções escolhidas
def inputsmenu():
    x = float(input("Digite um número\n"))
    y = float(input("Digite um outro número\n"))
    menu(x, y)

# segundo menu onde se calcula operações basicas de matemática
def menu(x, y):
    escolha = int(input('''
...
# opção pra indentificar se e linear
def linear(y, a, x, b):
    if type(y) == str:
        print("y=%d*%d+%d" % (a, x, b))
...
# opção para indentificar se e oar ou impar
def dualidade():
...
# opção para indentificar se sao numeros multiplos
def mult():
